...
export async function createShopOrder(lines: CartLine[], address: ShippingAddress) {
  ...
  const normalized = lines
    .map((line) => ({ id: String(line.id), quantity: Math.floor(Number(line.quantity)) }))
    .filter((line) => line.quantity > 0 && line.quantity <= 9999)
  if (!normalized.length) return { ok: false as const, error: 'empty_cart' }

  const ids = [...new Set(normalized.map((line) => line.id))]
  const { data: products, error: productsError } = await supabase
    .from('shop_products')
    .select('id, name, price_kurus, max_quantity, in_stock')
    .in('id', ids)
  ...
  const { data: order, error: orderError } = await supabase.from('tma_orders').insert({
    order_number: String(numberData), user_id: user.id, subtotal_kurus: subtotal, shipping_kurus: shipping, total_kurus: subtotal + shipping,
    phone_snapshot: address.phone, address_snapshot: address, current_phone: address.phone, current_address: address,
