...
// Links a published article to a purchasable product. One article links to
// at most one product; one product may have several linked articles.
export type ArticleProductLink = { articleId: string; productId: string }

export async function fetchProductForArticle(articleId: string | number): Promise<ShopProduct | null> {
  if (!isSupabaseConfigured) return null
  const { data: link } = await supabase.from('tma_article_products').select('product_id').eq('article_id', String(articleId)).maybeSingle()
  if (!link) return null
  return fetchProductById(link.product_id as string)
}

export async function fetchProductById(productId: string): Promise<ShopProduct | null> {
  if (!isSupabaseConfigured) return null
  const { data: product } = await supabase.from('shop_products').select('*').eq('id', productId).maybeSingle()
  return product ? mapProduct(product as ProductRow) : null
}

export async function fetchArticleIdsForProduct(productId: string): Promise<string[]> {
  if (!isSupabaseConfigured) return []
  const { data } = await supabase.from('tma_article_products').select('article_id').eq('product_id', productId)
  return (data ?? []).map((row) => row.article_id as string)
}

export async function fetchAllArticleProductLinks(): Promise<ArticleProductLink[]> {
  if (!isSupabaseConfigured) return []
  const { data } = await supabase.from('tma_article_products').select('article_id, product_id')
  return (data ?? []).map((row) => ({ articleId: row.article_id as string, productId: row.product_id as string }))
}

export async function setArticleProductLink(articleId: string, productId: string | null): Promise<{ error: string | null }> {
  if (!isSupabaseConfigured) return { error: 'Supabase yapılandırılmamış.' }
  if (!productId) {
    const { error } = await supabase.from('tma_article_products').delete().eq('article_id', articleId)
    return { error: error?.message ?? null }
  }
  const { error } = await supabase.from('tma_article_products').upsert({ article_id: articleId, product_id: productId }, { onConflict: 'article_id' })
  return { error: error?.message ?? null }
}
