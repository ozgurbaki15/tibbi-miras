...
export function CommunityCommerceManager() {
  ...
  async function createMembershipCode(event: React.FormEvent<HTMLFormElement>) {
    ...
    const { error } = await supabase.from('tma_membership_grants').insert({ code: normalized, tier: grantTier, duration_days: days, max_uses: 1, active: true })
