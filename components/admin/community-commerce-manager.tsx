...
export function CommunityCommerceManager() {
  ...
  const [grantTier, setGrantTier] = useState<'premium' | 'platin'>('premium')
  ...
  async function createMembershipCode(event: React.FormEvent<HTMLFormElement>) {
  ...
  return (
    <div className="mt-8 grid gap-6 lg:grid-cols-2">
      ...
      <section className="rounded-md border border-border bg-card p-6"><h2 className="mb-4 font-serif text-2xl text-card-foreground">Üyelik hediye kodu</h2><form onSubmit={createMembershipCode} className="flex flex-col gap-4"><label className="font-sans text-sm text-card-foreground">Kullanıcı UUID (boş bırakılırsa kod olarak üretilir)<input value={grantUserId} onChange={(event) => setGrantUserId(event.target.value)} className="mt-1.5 w-full rounded-md border border-border bg-background px-3 py-2 font-mono text-sm outline-none focus:border-primary" placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" /></label><label className="font-sans text-sm text-card-foreground">Kod<input value={grantCode} onChange={(event) => setGrantCode(event.target.value)} required maxLength={40} className="mt-1.5 w-full rounded-md border border-border bg-background px-3 py-2 uppercase outline-none focus:border-primary" /></label><label className="font-sans text-sm text-card-foreground">Üyelik<select value={grantTier} onChange={(event) => setGrantTier(event.target.value as 'premium' | 'platin')} className="mt-1.5 w-full rounded-md border border-border bg-background px-3 py-2 outline-none focus:border-primary"><option value="premium">Premium</option><option value="platin">Platin</option></select></label><label className="font-sans text-sm text-card-foreground">Geçerlilik (gün)<input type="number" min="1" step="1" value={durationDays} onChange={(event) => setDurationDays(event.target.value)} required className="mt-1.5 w-full rounded-md border border-border bg-background px-3 py-2 outline-none focus:border-primary" /></label><button type="submit" className="rounded-md bg-primary px-4 py-3 font-sans text-xs uppercase tracking-wider text-primary-foreground">Üyelik kodu üret</button></form></section>
    </div>
  )
}
