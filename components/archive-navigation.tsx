    ...
    <nav aria-label={lang === 'tr' ? 'Ana navigasyon' : 'Main navigation'} className="legend-nav sticky top-0 z-30 border-b border-primary/30 backdrop-blur">
      <div className="mx-auto flex max-w-6xl gap-1 overflow-x-auto px-6 py-3">
        {items.map(({ href, label, icon: Icon }) => (
          <Link key={href} href={href} scroll onClick={() => window.scrollTo({ top: 0, behavior: 'instant' })} className="legend-nav-link inline-flex shrink-0 items-center gap-2 rounded-md px-3 py-2 font-sans text-xs uppercase tracking-wider text-primary-foreground/75 transition-colors hover:bg-primary/15 hover:text-primary">
