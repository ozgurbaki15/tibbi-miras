...
export function ArticleTabs({ article }: { article: Article }) {
  ...
  const premium = active === 'tr' ? article.premium_content_tr : article.premium_content_en
  ...
  return <div>
    ...
    {premium ? <section className="relative overflow-hidden rounded-md border border-primary/30 bg-card p-6"><div className="pointer-events-none select-none blur-sm" aria-hidden="true"><p className="font-serif text-lg leading-relaxed text-foreground/60">{premium.slice(0, 280)}</p></div><div className="absolute inset-0 flex flex-col items-center justify-center bg-card/75 text-center"><Lock className="mb-2 size-5 text-primary" /><h3 className="font-serif text-xl text-foreground">Premium devamı</h3><p className="mt-1 max-w-xs font-sans text-xs leading-relaxed text-muted-foreground">{user ? 'Bu eser üyelik veya tekil erişim gerektirir.' : 'Devamını okumak için giriş yapın.'}</p><Link href={user ? '/settings' : '/login'} className="mt-4 rounded-md bg-primary px-4 py-2 font-sans text-xs uppercase tracking-wider text-primary-foreground">{user ? 'Erişim seçenekleri' : 'Giriş yap'}</Link></div></section> : null}
  </div>
}
