...
type LinkRow = { article_id: string; product_id: string; article_first: boolean; product_first: boolean }
...
export function ArticleProductLinks({ articleId }: { articleId: string }) {
  ...
  useEffect(() => {
    let active = true
    supabase.from('tma_article_product_links').select('article_id, product_id, article_first, product_first').eq('article_id', articleId).then(({ data, error }) => {
      if (!active) return
      if (error) {
  ...
  useEffect(() => {
    const articleIds = links.map((link) => link.article_id).filter(Boolean)
    if (!articleIds.length) return
    supabase.from('articles').select('id, title_tr, title_en').in('id', articleIds).then(({ data }) => {
      setRelatedArticles((data as RelatedArticle[] | null) ?? [])
    })
  }, [links])
  ...
  return (
    <section className="mt-12 rounded-2xl border border-primary/35 bg-card/80 p-5 shadow-xl backdrop-blur-sm md:p-7" aria-labelledby="related-products-heading">
      ...
      <div className="grid gap-4 sm:grid-cols-2">{linkedProducts.map(({ product }) => product ? <article key={product.id} className="overflow-hidden rounded-xl border border-border bg-background/70"><div className="flex gap-4 p-4">{product.image ? <img src={product.image} alt={product.name} className="size-20 rounded-lg object-cover" /> : null}<div className="min-w-0 flex-1"><h3 className="font-serif text-lg text-foreground">{lang === 'tr' ? product.name : product.nameEn || product.name}</h3><p className="mt-1 font-sans text-sm text-muted-foreground">{shopPriceLabel(product.priceKurus, lang)}</p><button type="button" onClick={() => add(product.id)} disabled={!product.inStock} className="mt-3 inline-flex items-center gap-2 rounded-lg bg-primary px-3 py-2 font-sans text-xs font-semibold uppercase tracking-wider text-primary-foreground disabled:opacity-50"><ShoppingCart className="size-3.5" /> {product.inStock ? (lang === 'tr' ? 'Sepete ekle' : 'Add to cart') : (lang === 'tr' ? 'Tükendi' : 'Out of stock')}</button>{relatedArticles.length ? <div className="mt-3 border-t border-border/70 pt-3"><p className="mb-2 font-sans text-xs uppercase tracking-wider text-muted-foreground">{lang === 'tr' ? 'Bu ürün hakkında' : 'About this product'}</p>{relatedArticles.map((article) => <Link key={article.id} href={`/article/${article.id}`} className="inline-flex items-center gap-1 font-sans text-sm font-semibold text-primary underline-offset-4 hover:underline">{lang === 'tr' ? article.title_tr : article.title_en || article.title_tr}<ArrowRight className="size-3.5" aria-hidden="true" /></Link>)}</div> : null}</div></div></article> : null)}</div>
    </section>
  )
}
